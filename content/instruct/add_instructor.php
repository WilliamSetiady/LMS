...
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title"><?= isset($_POST['save']) ? "Save" : "Edit" ?></h5>
                <form action="" method="post">
                    ...
                    <div class="mb-3">
                        <input type="radio" name="instructor_gender" value="1" <?= (isset($_GET['edit']) && $rowEdit['instructor_gender'] == '1') ? 'checked' : '' ?> required> Laki-Laki
                        <input type="radio" name="instructor_gender" value="0"<?= (isset($_GET['edit']) && $rowEdit['instructor_gender'] == '0') ? 'checked' : '' ?> required> Perempuan
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">Instructor Education *</label>
                        <input type="text" class="form-control" name="instructor_education" placeholder="Enter your Name"
                            value="<?= isset($_GET['edit']) ? $rowEdit['instructor_education'] : '' ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">Instructor Phone *</label>
                        <input type="text" class="form-control" name="instructor_phone" placeholder="Enter your Name"
                            value="<?= isset($_GET['edit']) ? $rowEdit['instructor_phone'] : '' ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">Email *</label>
                        <input type="email" class="form-control" name="instructor_email" placeholder="Enter your Email"
                            value="<?= isset($_GET['edit']) ? $rowEdit['instructor_email'] : '' ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">Address *</label>
                        <input type="text" class="form-control" name="instructor_address" placeholder="Enter your Email"
                            value="<?= isset($_GET['edit']) ? $rowEdit['instructor_address'] : '' ?>" required>
                    </div>
                    <div class="mb-3">
                        <input type="submit" class="btn btn-success" name="<?= isset($instructorId) && $instructorId != '' ? 'edit' : 'save'; ?>" value="<?= isset($instructorId) && $instructorId != '' ? 'Update' : 'Save'; ?>">
                    </div>
